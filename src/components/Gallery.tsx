    ...
    <section id="gallery" className="py-24 bg-black relative overflow-hidden">
      <div className="container mx-auto px-4">
        <div className="max-w-4xl mx-auto text-center mb-16">
          ...
          <p className="text-white/80 text-lg">A showcase of our artistry and craft in mixology</p>
          
          <div className="flex justify-center mt-8 mb-12 space-x-4 flex-wrap">
            {['All', 'Cocktails', 'Events', 'Services'].map((category) => (
              <button
                key={category}
                onClick={() => setFilter(category)}
                className={`px-4 py-2 rounded-full transition-all ${
                  filter === category 
                    ? 'bg-cocktail-gold text-black' 
          ...
          {filteredImages.map((image, index) => (
            ...
            >
              ...
              <div className="absolute inset-0 bg-gradient-to-t from-black/80 to-transparent opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex flex-col justify-end p-6">
                <h3 className="text-xl font-bold text-white mb-1 font-['Playfair_Display']">{image.title}</h3>
                <p className="text-cocktail-gold">{image.category}</p>
              </div>
            </motion.div>
          ))}
        </div>
